...
- 訂單已完成領料後若要修改品項/數量或刪除訂單，必須先走兩段式回料：業務端只能送出 `回料申請`，ERP 先把領料主單與明細標為 `待回料確認`，這一步不修改庫存；倉管或管理者確認實體物料已回倉後，ERP 才透過 Supabase 批次庫存交易加回數量，寫入 `領料沖銷` 異動紀錄，再把領料主單與明細標為 `已沖銷`。`已沖銷`/`已退料` 領料單不再阻擋訂單修改或刪除；若 Notion 鏡像失敗，只排入補同步，不重跑庫存交易。
- 若蝦皮補庫訂單已完成並增加 S- 成品庫存，刪除訂單前還必須走「成品入庫沖銷」：ERP 會從異動紀錄找出 `蝦皮完成入庫` 的 S- 料號與數量，透過 Supabase 批次庫存交易扣回，寫入 `入庫沖銷` 異動紀錄，然後才允許刪除。領料沖銷與成品入庫沖銷是兩段不同庫存影響，不能只完成其中一段就刪除訂單。
...
- 已完成 C端訂單不再用取消表示退貨；退貨使用獨立入口，狀態改為 `部分退貨` 或 `已退貨`。
- 退貨原因使用固定分類：`買錯/不需要該產品`、`故障`、`寄錯產品`、`其他`；選 `其他` 時必須填寫原因，供日後退貨率與原因分析。
- 選擇回補庫存時，先透過既有 Supabase 庫存批次路徑回補 S- 成品，再寫入 `C端退料` 異動紀錄；不回補時只更新 C端訂單狀態與備註。
- C端取消或刪除前會先稽核 `C端出貨` 與 `C端退料` 異動紀錄，計算尚未回補的 S- 成品數量；出貨中訂單可選回庫後取消/刪除或報銷不回庫，已完成訂單優先引導到退貨流程保留成交與退貨原因，只有建錯或測試資料才建議刪除。
- 每次退貨使用獨立退貨批號，避免同一訂單多次部分退貨被庫存防重鍵誤判為重複。
- 部分退貨會依 C端訂單備註中的既有退貨紀錄計算已退數量，限制剩餘可退數量，避免累計退貨超過原訂單數量。
- 若庫存回補已成功但 C端訂單 Notion 狀態暫時更新失敗，狀態與備註更新會進入既有流程補同步佇列，不可再次用庫存交易重跑來修復。
